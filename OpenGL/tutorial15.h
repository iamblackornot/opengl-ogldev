...
public:
	...
	Result Init() override
	{
		...
		Camera camera;
		//auto strafe = - glm::cross(camera.GetDirection(), camera.GetTilt());
		//std::cout << "direction " << camera.GetDirection() << std::endl;
		//std::cout << "tilt " << camera.GetTilt() << std::endl;
		//std::cout << "strafe " << strafe << std::endl;

		//auto tilt = glm::cross(camera.GetDirection(), strafe);
		//std::cout << "direction " << camera.GetDirection() << std::endl;
	...
	{
		_pipeline.OnKeyEvent(key, scancode, action, mods);
	}

	void SetScreenResolution(int width, int height) override
	{
		_pipeline.SetResolution(width, height);
	}
private:
